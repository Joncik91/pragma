...
def _tautological_evidence(func: ast.FunctionDef) -> str:
    ...
    if not asserts:
        # `with pytest.raises(...)` *is* an assertion in pytest semantics
        # (the context manager fails the test if no matching exception
        # fires). Don't flag those bodies as tautological.
        if _has_raises_assertion(func):
            return ""
