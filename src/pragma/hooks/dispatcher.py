...
def _safe_default(event: str) -> dict:
    if event in ("pre-tool-use", "post-tool-use"):
        return {"permissionDecision": "allow"}
    return {"continue": True}


def _write_json(stdout: IO[str], obj: dict) -> None:
...
def _reject_unknown(stdout: IO[str], event: str) -> int:
    _write_json(stdout, {
        ...
        "remediation": (
            f"Valid events: {sorted(_HANDLERS)}. "
...
def _reject_missing_stdin(stdout: IO[str], event: str) -> int:
    _write_json(stdout, {
        ...
        "remediation": (
            "This command is invoked by Claude Code hooks; "
...
def dispatch(
    *, event: str, stdin: IO[str], stdout: IO[str], cwd: Path | None,
) -> int:
