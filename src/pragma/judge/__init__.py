...
Asks a small model (DeepSeek by default; any OpenAI-compatible endpoint
via PRAGMA_LLM_BASE_URL) "does this test verify the production function's
behavior, or does it only test structure?" Emits `<lang>.semantic_gaming`
verdict (warning, not blocking) when the model says no.

Tier 3 is opt-in via `--with-llm` CLI flag and PRAGMA_LLM_API_KEY (or the
legacy PRAGMA_DEEPSEEK_API_KEY / PRAGMA_ANTHROPIC_API_KEY aliases).
...
Architecture:
    ...
    client.py    -> thin OpenAI-compatible client (DeepSeek default)
