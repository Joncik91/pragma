...
| **v1.1** | **Released 2026-04-24** | First-run PIL reachable end-to-end: `pragma slice complete` / `unlock` / `verify gate` emit `.pragma/pytest-junit.xml` automatically via an explicit `--junit-xml=` CLI flag that survives the v1.0.3 `-o addopts=` clear (REQ-016 / BUG-020); `pragma report` adds a `## Diagnostics` banner when junit.xml or `.pragma/spans/` is absent (REQ-017); the pre-release smoke script runs an end-to-end `activate → unlock → complete → report` cycle on a fresh greenfield and asserts the PIL populates (REQ-018); greenfield `claude.md` primer + concepts / reference docs explain the loop as it is (REQ-019). v1.0's one-bug-at-a-time patch cadence halts — this reframe was the honest v1.0 done-criteria. | A first-run user following the README ships a slice and sees a populated PIL with `verified > 0, missing = 0` without a separate manual `pytest` step. v1.0 criterion 3 (*PIL legible to 3 non-coders*) becomes a question of legibility not reachability. | `scripts/pre-release-smoke.sh` asserts junit.xml exists and `pragma report --json summary.ok >= 1 && summary.missing == 0` after `slice complete` on a fresh greenfield |
| **v1.2** | Planned | (1) `pragma-sdk-ts` + TS pre-commit battery template (biome + tsc-strict + npm-audit) + TS discipline-checker (originally v1.1); (2) human-subjects PIL legibility validation on 3 non-coders (closes v1.0 criterion 3); (3) `pragma report --human` distinguishes "first-run, no artifacts" from "artifacts present but real coverage gap" via richer banner copy. PyPI publish deferred. | First non-Python target ships on a now-reliable Python base. | TS project can `pragma init` and run the full gate + battery + PIL; 3 non-coders read a populated PIL without coaching |
| **v1.3** | Candidate | `pragma context <slice>` — manifest-as-context-selector. Given a slice id, emit the minimal file set (sources covered by the slice's permutations + their tests + the manifest entries) needed to work on it. Consumed by Claude Code / other LLM harnesses to bound session context on large codebases. Also shrinks the surface `verify discipline` and `pytest` scan per commit. | Turns the manifest from a gate into a context-selection tool. Uniquely Pragma's — only works because the manifest already maps requirements → source → tests. Biggest lever on large-codebase token cost. | `pragma context <slice>` output is stable, covers every file the slice's tests touch at runtime, and missing coverage triggers a doctor-reported diagnostic |

**Total plan: ~4 weeks** across three remaining releases. v0.3 ~1.5w,
v0.4 ~1.5w, v1.0 ~1w.

**Beyond v1.0:** v1.1 *was* planned as TypeScript but became
"first-run PIL reachable" after the v1.0.6 dogfood showed the
thesis wasn't actually reachable for a new user. Prioritising
reachability over new-language reach was the right call — TypeScript
shipping on a base where non-Pragma-author users can't get a
populated PIL would have compounded the problem. TypeScript is
v1.2. Context-selector (`pragma context`) is v1.3. Order: fix the
promise, then extend it.
